...
# ============================================================
# EDGE X PRO
# UPCOMING EVENT INTELLIGENCE V6.3
# ============================================================
...
def build_horizon_predictions(matched_patterns):
    ...
    for horizon in HORIZONS:
        ...
        # Evidence quality for the CURRENT horizon must reflect the amount
        # of matching evidence available now. A single historical pattern
        # must never be labelled STRONG.
...
def build_fundamental_summary(setup):
    ...
    markets = fundamental.get("markets", {})
    if not isinstance(markets, dict):
        markets = {}

    macro = fundamental.get("macro", {})
    if not isinstance(macro, dict):
        macro = {}

    core_cpi = macro.get("core_cpi", {})
    if not isinstance(core_cpi, dict):
        core_cpi = {}
    ...
    # ========================================================
    # CORE CPI RELEASE EXPECTATION
    #
    # For a CPI event, the verified consensus-vs-actual surprise is the
    # primary fundamental input. Before the release, if consensus is not
    # verified, we do NOT invent a directional surprise.
    # ========================================================

    core_actual_mom = as_float(core_cpi.get("actual_mom"))
    core_actual_yoy = as_float(core_cpi.get("actual_yoy"))
    core_forecast = as_float(
        core_cpi.get("forecast_mom", core_cpi.get("forecast"))
    )
    core_previous = as_float(
        core_cpi.get("previous_mom", core_cpi.get("previous"))
    )

    inputs["core_cpi_actual_mom"] = core_actual_mom
    inputs["core_cpi_actual_yoy"] = core_actual_yoy
    inputs["core_cpi_forecast_mom"] = core_forecast
    inputs["core_cpi_previous_mom"] = core_previous

    consensus_available = core_forecast is not None
    inputs["verified_core_cpi_consensus_available"] = consensus_available

    if consensus_available and core_actual_mom is not None:
        # This branch is useful after a release when the actual and verified
        # consensus are both present.
        surprise = core_actual_mom - core_forecast
        inputs["core_cpi_surprise_mom"] = round(surprise, 4)
        maximum_score += 5.0

        if surprise <= -0.05:
            score += 5.0
            reasons.append(
                "Core CPI printed below verified consensus, a direct bullish input for gold."
            )
        elif surprise >= 0.05:
            score -= 5.0
            reasons.append(
                "Core CPI printed above verified consensus, a direct bearish input for gold."
            )
        else:
            reasons.append(
                "Core CPI was broadly in line with verified consensus."
            )
    else:
        reasons.append(
            "Verified Core CPI consensus is unavailable, so no release-surprise direction is assumed."
        )

    # ========================================================
    # INFLATION MOMENTUM
    #
    # Supporting context only. It must not be treated as a forecast of the
    # next CPI surprise.
    # ========================================================

    inflation = normalize(
        fundamental.get("inflation_trend", "UNKNOWN")
    )
    inputs["inflation_trend"] = inflation

    if inflation not in ("", "UNKNOWN"):
        maximum_score += 2.0

        if inflation == "COOLING":
            score += 1.5
            reasons.append(
                "Cooling inflation momentum is a supporting bullish backdrop for gold."
            )
        elif inflation == "RISING":
            score -= 1.5
            reasons.append(
                "Rising inflation momentum is a supporting bearish backdrop for gold."
            )
        else:
            reasons.append(
                "Inflation momentum is mixed and adds little directional edge."
            )

    # ========================================================
    # HEADLINE CPI CONTEXT
    # ========================================================

    headline_mom = as_float(cpi.get("actual_mom"))
    headline_yoy = as_float(cpi.get("actual_yoy"))
    inputs["headline_cpi_actual_mom"] = headline_mom
    inputs["headline_cpi_actual_yoy"] = headline_yoy

    # ========================================================
    # DXY - SUPPORTING MARKET CONTEXT
    # ========================================================
    ...
    dxy_direction = normalize(
        dxy.get(
            "direction",
            fundamental.get("dollar_bias", "UNKNOWN"),
        )
    )

    inputs["dxy_direction"] = dxy_direction
    inputs["dxy_value"] = dxy.get("value")

    if dxy_direction not in ("", "UNKNOWN"):
        maximum_score += 1.5

        if dxy_direction == "FALLING":
            score += 1.5
            reasons.append(
                "A falling US dollar supports gold into the CPI event."
            )
        elif dxy_direction == "RISING":
            score -= 1.5
            reasons.append(
                "A rising US dollar pressures gold into the CPI event."
            )
        else:
            reasons.append(
                "The US dollar trend is mixed and provides little CPI-event edge."
            )

    # ========================================================
    # US10Y - SUPPORTING MARKET CONTEXT
    # ========================================================

    us10y = markets.get("us10y", {})
    if not isinstance(us10y, dict):
        us10y = {}

    yield_direction = normalize(
        us10y.get(
            "direction",
            fundamental.get("treasury_yields", "UNKNOWN"),
        )
    )

    inputs["us10y_direction"] = yield_direction
    inputs["us10y_value"] = us10y.get("value")

    if yield_direction not in ("", "UNKNOWN"):
        maximum_score += 1.5

        if yield_direction == "FALLING":
            score += 1.5
            reasons.append(
                "Falling Treasury yields support gold into the CPI event."
            )
        elif yield_direction == "RISING":
            score -= 1.5
            reasons.append(
                "Rising Treasury yields pressure gold into the CPI event."
            )
        else:
            reasons.append(
                "Treasury yields are mixed and provide little CPI-event edge."
            )
    ...
    if maximum_score <= 0:
        return {
            ...
            "maximum_score": 0.0,
            "reasons": [
                "Insufficient verified Core CPI fundamental data."
            ],
            "inputs": inputs,
            "consensus_available": consensus_available,
            "confidence_cap": 54.0 if not consensus_available else 75.0,
        }

    normalized_score = score / maximum_score

    # Require a meaningful directional imbalance. Supporting market context
    # alone should rarely create a strong pre-release fundamental call.
    ...
    confidence = 50.0 + abs(normalized_score) * 30.0

    # Without a verified consensus there is no defensible way to predict the
    # actual Core CPI surprise. Cap fundamental confidence accordingly.
    confidence_cap = 75.0 if consensus_available else 54.0
    confidence = min(confidence_cap, max(50.0, confidence))

    # If the capped confidence is too low, keep the output neutral even when
    # supporting context leans in one direction.
    if confidence < 55.0:
        bias = "NO_CLEAR_EDGE"
    ...
    return {
        ...
        "method": (
            "CORE_CPI_SURPRISE_PLUS_MARKET_CONTEXT"
            if consensus_available
            else "CORE_CPI_CONTEXT_ONLY_NO_VERIFIED_CONSENSUS"
        ),
    }
...
def main():
    ...
    print(
        "CORE CPI UPCOMING INTELLIGENCE V2"
    )
    ...
    technical_summary["historical_matches"] = len(matched_patterns)
    technical_summary["horizons"] = horizon_predictions
    ...
    output = {
        ...
        "consensus_status":
            "PENDING_VERIFIED_HISTORICAL_SOURCE",
