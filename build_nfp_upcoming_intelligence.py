...
def build_fundamental_summary(setup):
    ...
    if not isinstance(unemployment, dict):
        unemployment = {}
    ...
    # NFP surprise is the primary release-specific input when a verified
    # consensus exists. Before release, no surprise direction is invented.
    nfp_actual = as_float(nfp.get("actual"))
    nfp_previous = as_float(nfp.get("previous"))
    nfp_forecast = as_float(nfp.get("forecast"))

    inputs["nfp_actual"] = nfp_actual
    inputs["nfp_previous"] = nfp_previous
    inputs["nfp_forecast"] = nfp_forecast

    consensus_available = nfp_forecast is not None
    inputs["verified_nfp_consensus_available"] = consensus_available

    if consensus_available and nfp_actual is not None:
        surprise = nfp_actual - nfp_forecast
        inputs["nfp_surprise_thousand_jobs"] = round(surprise, 1)
        maximum_score += 5.0

        # Stronger jobs data generally supports USD/yields and pressures gold.
        if surprise >= 25.0:
            score -= 5.0
            reasons.append(
                "NFP printed materially above verified consensus, a direct bearish input for gold."
            )
        elif surprise <= -25.0:
            score += 5.0
            reasons.append(
                "NFP printed materially below verified consensus, a direct bullish input for gold."
            )
        else:
            reasons.append(
                "NFP was broadly close to verified consensus."
            )
    else:
        reasons.append(
            "Verified NFP consensus is unavailable, so no release-surprise direction is assumed."
        )

    # Labor trend is supporting context, not a substitute for consensus.
    labor_market = normalize(
        fundamental.get("labor_market", "UNKNOWN")
    )
    inputs["labor_market"] = labor_market

    if labor_market not in ("", "UNKNOWN"):
        maximum_score += 2.0

        if labor_market == "WEAKENING":
            score += 1.5
            reasons.append(
                "A weakening labor backdrop supports easier Fed expectations and is supportive for gold."
            )
        elif labor_market == "STRENGTHENING":
            score -= 1.5
            reasons.append(
                "A strengthening labor backdrop supports tighter policy expectations and can pressure gold."
            )
        else:
            reasons.append(
                "The labor-market trend is mixed and adds little directional edge."
            )

    unemployment_actual = as_float(unemployment.get("actual"))
    unemployment_previous = as_float(unemployment.get("previous"))
    unemployment_forecast = as_float(unemployment.get("forecast"))

    inputs["unemployment_actual"] = unemployment_actual
    inputs["unemployment_previous"] = unemployment_previous
    inputs["unemployment_forecast"] = unemployment_forecast

    # Unemployment is secondary context. Use verified forecast if present;
    # otherwise only a small previous-month trend contribution.
    if unemployment_actual is not None:
        ...
        reference = (
            ...
            else unemployment_previous
        )

        if reference is not None:
            change = unemployment_actual - reference

            if change >= 0.1:
                score += 1.0
                reasons.append(
                    "Higher unemployment than the comparison level is supportive for gold."
                )
            elif change <= -0.1:
                score -= 1.0
                reasons.append(
                    "Lower unemployment than the comparison level can pressure gold."
                )
            else:
                reasons.append(
                    "Unemployment is broadly unchanged and adds little directional edge."
                )
    ...
    dxy_direction = normalize(
        dxy.get("direction", fundamental.get("dollar_bias", "UNKNOWN"))
    )
    ...
    if dxy_direction not in ("", "UNKNOWN"):
        ...
        if dxy_direction == "FALLING":
            score += 1.5
            reasons.append(
                "A falling US dollar supports gold into the NFP event."
            )
        elif dxy_direction == "RISING":
            score -= 1.5
            reasons.append(
                "A rising US dollar pressures gold into the NFP event."
            )
        else:
            reasons.append(
                "The US dollar trend is mixed and provides little NFP-event edge."
            )
    ...
    yield_direction = normalize(
        us10y.get("direction", fundamental.get("treasury_yields", "UNKNOWN"))
    )
    ...
    if yield_direction not in ("", "UNKNOWN"):
        ...
        if yield_direction == "FALLING":
            score += 1.5
            reasons.append(
                "Falling Treasury yields support gold into the NFP event."
            )
        elif yield_direction == "RISING":
            score -= 1.5
            reasons.append(
                "Rising Treasury yields pressure gold into the NFP event."
            )
        else:
            reasons.append(
                "Treasury yields are mixed and provide little NFP-event edge."
            )
    ...
    if maximum_score <= 0:
        return {
            ...
            "score": 0.0,
            "maximum_score": 0.0,
            "reasons": ["Insufficient verified NFP fundamental data."],
            "inputs": inputs,
    ...
    return {
        ...
        "method": (
            "NFP_SURPRISE_PLUS_LABOR_AND_MARKET_CONTEXT"
            if consensus_available
            else "NFP_CONTEXT_ONLY_NO_VERIFIED_CONSENSUS"
        ),
    }
